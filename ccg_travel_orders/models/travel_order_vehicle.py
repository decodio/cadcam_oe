...
class TravelOrderVehicle(models.Model):
    _name = 'travel.order.vehicle'
    ...
    def name_get(self, cr, user, ids, context=None):
        ret = []
        for id in ids:
            vehicle_obj = self.browse(cr, user, id, context=context)
            if vehicle_obj.registration:
                ret.append((id, '{} [{}]'.format(vehicle_obj.name, vehicle_obj.registration)))
        return ret
